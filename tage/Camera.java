...
* The default camera position is at (0,0,1) looking down the -Z axisUpVector towards the origin.
* @author Scott Gordon
*/
...
public class Camera
{
	...
	/** instantiates a Camera object at location (0,0,1) and facing down the -Z axisUpVector towards the origin */
	public Camera()
	...
	public void pitch(float speed) {
		Vector3f rightVector = getU(); 
		Vector3f upVector = getV(); 
		Vector3f fwdVector = getN(); 
		upVector.rotateAxis(speed, rightVector.x(), rightVector.y(), rightVector.z()); 
		fwdVector.rotateAxis(speed, rightVector.x(), rightVector.y(), rightVector.z()); 
		setV(upVector); 
		setN(fwdVector); 			
	}

	/**
	 * turn left/right (yaw) around a certain rotation Matrix
	 */
	// Local yaw
	public void yaw(float speed) {
		Vector3f rightVector = getU(),
				 upVector = getV(),
				 fwdVector = getN();
		rightVector.rotateAxis(speed, upVector.x(), upVector.y(), upVector.z()); 
		fwdVector.rotateAxis(speed, upVector.x(), upVector.y(), upVector.z());
		upVector.rotateAxis(speed, upVector.x(), upVector.y(), upVector.z()); 
	...
	// Global yaw
	public void yaw(float speed, Vector3f axisUpVector) {
		Vector3f rightVector = getU(),
				 upVector = getV(),
				 fwdVector = getN();
		rightVector.rotateAxis(speed, axisUpVector.x(), axisUpVector.y(), axisUpVector.z()); 
		fwdVector.rotateAxis(speed, axisUpVector.x(), axisUpVector.y(), axisUpVector.z());
		upVector.rotateAxis(speed, axisUpVector.x(), axisUpVector.y(), axisUpVector.z()); 
