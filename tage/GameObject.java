...
public class GameObject
{
	...
	*/
	...
	// Does a yaw around a certain rotation Matrix
	// we can pass in the world y axis as necessary to make this global
	public void pitch(float speed) {
		Matrix4f dolM = getLocalRotation();
		// Vector3f rightVector = getWorldRightVector();
		Vector3f fwdVector = getLocalForwardVector(); 
		Vector3f upVector = getLocalUpVector(); 
		Vector3f rightVector = getLocalRightVector();  //rotate around this axis
		...
		dolM.setColumn(1, new Vector4f(upVector, 0f));
		dolM.setColumn(2, new Vector4f(fwdVector, 0f));

		setLocalRotation(dolM);
	}

	public void yaw(float speed, Matrix4f rotationMat) {
		Matrix4f dolM = getLocalRotation();
		rotationMat.rotateY(speed);
		dolM.mul(rotationMat);
		setLocalRotation(dolM);
	}
