...
{	
	...
	public String getViewportName() { return viewportName; }
	...
	// This will ensure that we get teh left-side position relative to the Canvas only
	// getActuaLeft() will also include the left position of the canvas window in the return
	public float getLeftOnCanvas()
	{	RenderSystem rs = engine.getRenderSystem();
		GLCanvas canvas = rs.getGLCanvas();
		Point loc = canvas.getLocationOnScreen();
		// float left = (float) loc.getX();
		float width = canvas.getWidth();
		return width * relativeLeft;
	}
